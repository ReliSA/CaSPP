[Home](../README.md) > [Catalogue](../Patterns_catalogue.md) > Ask for Individual Grading

# Ask for Individual Grading (Draft)

## [Classification](facets/facets.md)

- [Category](facets/categories/categories.md):
- [Form](facets/forms/forms.md): [Activity](facets/categories/Activity.md)
- [Methodology](facets/methodologies/methodologies.md): [General](facets/methodologies/General.md)
- [Mode](facets/modes/modes.md): [General](facets/modes/General.md)
- [Primary perspective](facets/perspectives/perspectives.md): [Individual Student](facets/perspectives/Individual_Student.md)
- [Stage](facets/stages/stages.md): [Grading](facets/categories/Grading.md)

## Context

## Problem

Some members of the group did not contribute as much to the group results as other team members, but this is not reflected in the groups’ grade.

## Forces

## Solution

Ensure that the different degrees of contribution are taken into account by asking for (at least partially) individual grading.

## Consequences

## Related Patterns

|Pattern|Relation type|Relation description|
|--|--|--|
|[Give a First Warning](Give_a_First_Warning.md)|can follow||
|[Share Expectations](Share_Expectations.md)|preceded by||
 
## Example(s) / Known Use(s)

## [Sources](../References.md)

[[KOP'12]](facets/publications/kop12/kop12.md)
