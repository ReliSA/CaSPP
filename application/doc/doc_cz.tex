...
\begin{titlepage}
    ...
    \begin{flushleft}
        ...
        Osobní čísla: A25N0040P, A25N0052P, A25N0056P, A25N0061P \\
        Emaily: qhdt@students.zcu.cz, bmilota@students.zcu.cz, reichm@students.zcu.cz, eliska@toncar.cz \\
        Datum: 16. 5. 2026
    \end{flushleft}
\end{titlepage}
...
Následující podsekce shrnují závislosti mezi moduly a tok dat při klíčových činnostech aplikace. 
Samotné diagramy jsou z důvodu přehlednosti přesunuty do \textbf{Přílohy \ref{app:diagramy}}. 
Diagramy zobrazení a analýzy zahrnují i vrstvy editoru a analyzátoru, aby byl vidět kontext práce 
se soubory v celém Markdown pohledu.

\subsubsection*{Diagram závislostí modulů}

Diagram \ref{fig:diagram-dependency} (viz Příloha \ref{app:diagramy}) znázorňuje, 
jak jsou jednotlivé třídy aplikace organizovány do vrstev a v jakém směru jdou importy. 
Vstupním bodem je \texttt{main.py}, které inicializuje centrální třídu \texttt{Application}. 
Ta drží referenci na všechny manažery (\texttt{FileManager} atd.) a na hlavní okno \texttt{MainWindow}. 
Manažeři přes UI komponenty (\texttt{MarkdownScene}, \texttt{TabWidget}, atd.) volají datovou vrstvu 
-- \texttt{MarkdownAnalyzer} se svými validátory, 
parsery (\texttt{MarkdownParser}, \texttt{TemplateParser} sdílející \texttt{BaseParser}), 
\texttt{FileHelper}, \texttt{FileMatcher} a balíček \texttt{utils.git}. Šipka \texttt{A $\rightarrow$ B} znamená, 
že modul A importuje (závisí na) modulu B; barvy odlišují vstupní bod, manažery, UI moduly a datové moduly.

\subsubsection*{Tok dat při Git operacích}

Diagramy \ref{fig:dataflow-git-a} a \ref{fig:dataflow-git-b} (viz Příloha \ref{app:diagramy}) popisují dva paralelní toky. 
\textbf{Flow A} (diagram \ref{fig:dataflow-git-a}) reprezentuje uživatelem spouštěné Git operace (\emph{status}, 
\emph{fetch}, \emph{pull}, \emph{push}, \emph{export staged}): kliknutí v \texttt{GitScene} nebo v menu projde 
přes \texttt{GitManager.\allowbreak start\_operation}, který vytvoří \texttt{GitOperationWorker} (\texttt{QThread}) 
a v něm spustí příslušnou funkci z balíku \texttt{utils.git} nad knihovnou GitPython. Výsledek (\texttt{GitResult}) 
se přes signály \texttt{operation\_output} a \texttt{operation\_finished} vrací zpět do GUI vlákna a vypisuje se
v konzoli \texttt{GitScene}. 

\textbf{Flow B} (diagram \ref{fig:dataflow-git-b}) ukazuje automatické stagování po uložení souboru: 
\texttt{FileManager} vyvolá \texttt{MarkdownAutoStager.\allowbreak stage\_file\_delayed}, který přes 
jednorázový \texttt{QTimer} (1\,s) spustí synchronní \texttt{stage\_\allowbreak markdown\_\allowbreak files} 
a o výsledku informuje signály \texttt{file\_\allowbreak staged} nebo \texttt{staging\_\allowbreak failed}.

\subsubsection*{Tok dat při zobrazení markdown souboru}

Diagram \ref{fig:dataflow-display} (viz Příloha \ref{app:diagramy}) ukazuje cestu od otevření souboru 
(toolbar, dvojklik v průzkumníku, obnovení session nebo proklik v náhledu) až do dvou souběžných pohledů. 
\texttt{FileManager.\allowbreak load\_markdown\_file} deleguje validaci a načtení bytů na \texttt{FileHelper} 
(s detekcí kódování) a předá obsah do \texttt{TabManager.\allowbreak load\_file\_into\_tab}. 
Z něj tečou data dvěma směry: do \texttt{MarkdownEditorWidget} (kde \texttt{MarkdownHighlighter} 
provádí zvýraznění syntaxe po blocích a vykresluje \emph{editor pane}) a -- přes signál \texttt{textChanged} 
a \texttt{EditorManager.\allowbreak update\_live\_preview} -- do knihovny \texttt{markdown} s rozšířeními \texttt{pymdownx}, 
jejíž HTML výstup je obalen výchozím CSS a zobrazen v \texttt{QTextBrowser} jako \emph{preview pane}.

\subsubsection*{Tok dat při analýze a validaci}

Diagram \ref{fig:dataflow-analysis} (viz Příloha \ref{app:diagramy}) ilustruje proces validace, který se 
spouští po načtení nebo uložení markdown souboru. \texttt{FileManager.\allowbreak \_analyze\_current\_file} 
paralelně získá dvě rozdílné věci: \texttt{ParsedDocument} z \texttt{MarkdownParser.\allowbreak parse\_content} 
a \texttt{TemplateRules} pro správnou šablonu, kterou \texttt{FileMatcher} dohledá podle rodičovského adresáře souboru. 
Následně obě data putují do \texttt{MarkdownAnalyzer.\allowbreak validate\_structure}, který spustí čtyři skupiny kontrol 
(\texttt{FormattingValidator}, \texttt{ContentValidator}, vlastní kontroly H1 a sekcí, \texttt{LinkValidator}). 
Výsledný \texttt{analysis} dict (\texttt{warnings} + \texttt{passed}) je převeden metodou \texttt{generate\_report} 
na text a \texttt{TabManager.\allowbreak set\_analysis} ho po řádcích zobrazí v panelu \texttt{analyzer\_list} 
jako barevně odlišené \texttt{QList\allowbreak Widget\allowbreak Item} pod editorem a náhledem.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{analyzer}} -- balíček obsahuje veškerou logiku pro strukturální, obsahovou
    ...
    \begin{itemize}
        ...
        Hlavní orchestrátor analýzy. Postupně spouští validátory
    ...
    \begin{itemize}
        ...
        Má na starosti řízení samotného textového editoru a jeho doplňkových vizuálních panelů. 
        ...
        Centralizuje zobrazení vyskakovacích dialogů a chybových hlášení napříč celou aplikací. 
        ...
        Koordinuje práci s dokumenty v uživatelském rozhraní: 
        ...
        Slouží jako správce pro operace s verzovacím systémem Git napojený na uživatelské rozhraní. 
        ...
        Spravuje životní cyklus záložek v (\texttt{MarkdownScene}). 
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        Třída \texttt{TemplateParser} představuje Analogie \texttt{MarkdownParser} pro šablonové soubory. 
        Výstupem je \texttt{TemplateRules}
        ...
        Třída \texttt{FileMatcher} páruje markdown soubor s odpovídající šablonou na základě jména rodičovského
        ...
        \texttt{InvalidInputError}, \ldots) s podporou srozumitelných hlášek a titulků
...
\begin{enumerate}
    \item \textbf{Parsování dokumentu a šablony.} \texttt{MarkdownParser} převede obsah markdown souboru
    ...
    spustí třídu \texttt{FormattingValidator} (formátování textu), \texttt{ContentValidator}
...
\begin{itemize}
    ...
    \item \textbf{Výstup analyzátoru (Analyzer Output):} Spodní panel v rámci záložky, který interaktivně vypisuje výsledky strukturální kontroly a validace odkazů. Barvami (červená, žlutá nebo zelená) odlišuje úspěšné kontroly, varování a kritické chyby.
\end{itemize}
...
Tato sekce popisuje proces a metodiku ověřování správné funkčnosti aplikace.

\subsection{Jednotkové testy}

Jednotkové testy desktopové aplikace jsou napsané v frameworku \texttt{pytest} a nachízejí se v adresáři
...
\begin{itemize}
    ...
    pokrývají třídu z core modulu \texttt{LinkValidator}: kontrolu existence cílů relativních odkazů,
...
\subsubsection*{Testy souborové vrstvy (\texttt{tests/file/})}

\begin{itemize}
    \item \textbf{\texttt{test\_paths.py}} --
    ověřuje výpočet kořenové cesty repozitáře v \texttt{Config} (vyhledání adresáře
    s~\texttt{.git} v aktuálním nebo nadřazených adresářích, chyba pokud Git repozitář neexistuje),
    výchozí cestu k~markdown souboru a inicializaci \texttt{MarkdownAnalyzer} vůči parametru \texttt{base\_path}.

    \item \textbf{\texttt{test\_file\_helper.py}} --
    pokrývá \texttt{FileHelper}: čtení a zápis s~UTF-8 a CP1252 (roundtrip),
    zamítnutí uložení nekompatibilních znaků při souboru s~ním nastaveným CP1252,
    \texttt{find\_markdown\_files} (s~rekurzí i bez, deduplikace),
    \texttt{get\_file\_info}, \texttt{resolve\_relative\_markdown\_link}, validaci souborů,
    limity velikosti (\texttt{MAX\_FILE\_SIZE\_MB}) a čtení/zápis včetně výjimek
    (\texttt{FileNotFoundError}, \texttt{FileSizeError}, \texttt{FileWriteError}).

    \item \textbf{\texttt{test\_file\_matcher.py}} --
    pokrývá \texttt{FileMatcher}: přiřazení šablony podle rodiče (či prarodiče) adresáře v~cestě k~souboru,
    složky katalogu (\texttt{catalogue/}, publications, categories metodiky apod.), indexové soubory
    pod \texttt{facets/} (šablony z~\texttt{facets\_index\_template}), pády pro neznámé cesty
    a cesty se zpětným lomítkem (\texttt{\textbackslash}).
\end{itemize}

\subsubsection*{Testy Git operací (\texttt{tests/git/})}

\begin{itemize}
    \item \textbf{\texttt{test\_git\_runner.py}} --
    testuje pomocné funkce v~\texttt{utils/git/runner.py}: normalizaci cesty vůči kořeni repozitáře,
    rozpoznání markdown přípon, nalezení kořene \texttt{.git} ze zanoření, filtrovaný Git status jen pro Markdown,
    formát výpisu statusu pro čistý i měněný working tree včetně staged změn.

    \item \textbf{\texttt{test\_git\_operations.py}} --
    pokrývá funkce pod \texttt{utils/git/} a chování \texttt{GitManager}: staging Markdown souborů
    a filtrování případně zadaných cest (\texttt{stage\_markdown\_files}, \texttt{stage\_file}),
    odstagging (\texttt{unstage\_all}), \texttt{commit} (prázdná či příliš dlouhá zpráva,
    žádné staged změny, režim \texttt{stage\_all}),
    \texttt{push\_markdown\_changes} (vlastní/z výchozí/z prázdné zprávě použití defaultu,
    odmítnutí příliš dlouhé commit zprávy, blokace když lokální větev zaostává za vzdálenou větví při mockovaném synchronizačním stavu),
    \texttt{fetch}/\texttt{pull}/\texttt{push} bez remotu či při problémovém sync stavu,
    export staged do ZIPu (\texttt{export\_staged\_files\_zip}), větev zamítnutí druhé paralelní operace
    u~\texttt{GitManager} při aktivním workeru,
    rozřazení neznámého příkazu a známých operací ve \texttt{GitOperationWorker}
    a větev závislá na uživatelském potvrzení u export staged.
\end{itemize}

\subsection{Testovací scénáře}

Pro komplexní ověření funkčnosti aplikace z pohledu uživatele, 
správného chování uživatelského rozhraní, ošetření chyb a integrace 
verzovacího systému Git byla vytvořena sada manuálních testovacích scénářů. 
Tyto scénáře pokrývají jak standardní průchody aplikací (tzv. happy paths), 
tak i nestandardní chování a okrajové případy. 

Kompletní výpis všech testovacích scénářů naleznete v \textbf{Příloze \ref{app:scenare}}.
...
\end{lstlisting}
    \item Sestavit balík podle předchozí podsekce z \textbf{kořene} repozitáře 
    (\texttt{build\_script.ps1} nebo \texttt{build\_script.sh}), nebo při ladění spouštět modul 
    z aktivovaného virtuálního prostředí v \texttt{application}, např.\ \texttt{python main.py}.
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \adjustbox{max width=\textwidth,max height=0.93\textheight,center}{\includegraphics{images/dependency_diagram.pdf}}
...
\begin{figure}[H]
    \centering
    \adjustbox{max width=\textwidth,max height=0.93\textheight,center}{\includegraphics{images/dataflow_git_a.pdf}}
    \caption{Tok dat při Git operacích (Flow A): akce uživatele v Git Scene nebo v menu, orchestrace v \texttt{GitManager}, provedení příkazů ve vlákně \texttt{GitOperationWorker} pomocí balíku \texttt{utils/git} a GitPython, zpětná vazba do konzole rozhraní.}
    \label{fig:dataflow-git-a}
\end{figure}

\begin{figure}[H]
    \centering
    \adjustbox{max width=\textwidth,max height=0.93\textheight,center}{\includegraphics{images/dataflow_git_b.pdf}}
    \caption{Tok dat při Git operacích (Flow B): automatické přidávání změn na pozadí (Auto-stager) spouštěné po uložení souboru.}
    \label{fig:dataflow-git-b}
\end{figure}

\begin{figure}[H]
    \centering
    \adjustbox{max width=\textwidth,max height=0.93\textheight,center}{\includegraphics{images/dataflow_display.pdf}}
...
\begin{figure}[H]
    \centering
    \adjustbox{max width=\textwidth,max height=0.93\textheight,center}{\includegraphics{images/dataflow_analysis.pdf}}
...
Tento dokument obsahuje hlavní funkční testy pro aplikaci, se zaměřením na uživatelské rozhraní, ošetření chyb a integraci Gitu.

\subsubsection*{1. Operace s Gitem (Git Operations)}

\textbf{Scénář 1.1: Provedení Git Push se zadanou commit zprávou} \\
...
\textbf{Scénář 1.2: Zamezení Git Push s prázdnou commit zprávou} \\
...
\textbf{Scénář 1.3: Export změněných souborů} \\
...
\textbf{Scénář 1.4: Zamezení nechtěné ztrátě dat při Git Exportu} \\
...
\subsubsection*{2. Ukončení aplikace a správa záložek (Tab Management)}

\textbf{Scénář 2.1: Zavírání aplikace s neuloženými záložkami (Cancel)} \\
...
\textbf{Scénář 2.2: Uložení neuložených záložek při zavírání aplikace} \\
...
\textbf{Scénář 2.3: Zahození změn při zavírání aplikace} \\
...
\subsubsection*{3. Průzkumník souborů a validace odkazů}

\textbf{Scénář 3.1: Bezpečné zpracování neplatných relativních odkazů} \\
...
\textbf{Scénář 3.2: Bezpečné ošetření smazání souboru mimo aplikaci} \\
...
\subsubsection*{4. Nastavení a uchování stavu (State Persistence)}

\textbf{Scénář 4.1: Zapamatování rozměrů a rozložení okna} \\
...
\textbf{Scénář 4.2: Obnovení dříve otevřených záložek} \\
...
\textbf{Scénář 4.3: Obnovení dříve otevřeného pracovního adresáře} \\
...
\subsubsection*{5. Živý editor a přepínače rozhraní}

\textbf{Scénář 5.1: Vykreslování živého náhledu v reálném čase} \\
...
\textbf{Scénář 5.2: Přepínání panelů uživatelského rozhraní (konkrétně "Analyzer")} \\
...
\textbf{Scénář 5.3: Přepínání panelů uživatelského rozhraní (konkrétně "Live Preview")} \\
\textbf{Popis:} Ověření, že UI čistě skrývá a obnovuje panely.
\begin{itemize}
    \item \textbf{DÁNO} uživatel má zobrazený hlavní pohled s markdownem.
...
\subsubsection*{6. Jádro Markdown analyzátoru}

\textbf{Scénář 6.1: Označení chybějících povinných nadpisů} \\
...
\textbf{Scénář 6.2: Analyzátor vymaže chyby po jejich opravě} \\
...
\subsubsection*{7. Integrace Průzkumníka souborů (File Explorer)}

\textbf{Scénář 7.1: Dvojité kliknutí otevře novou záložku} \\
...
\textbf{Scénář 7.2: Dvojité kliknutí na již otevřený soubor jej zaměří} \\
...
\subsubsection*{8. Automatické verzování na pozadí (Background Auto-Stager)}

\textbf{Scénář 8.1: Uložení souboru spustí auto-stager} \\
...
\subsubsection*{9. Horní menu a klávesové zkratky (Top Menu \& Shortcuts)}

\textbf{Scénář 9.1: Otevření souborů a složek přes File menu} \\
...
\textbf{Scénář 9.2: Ukládání pomocí klávesové zkratky} \\
...
\textbf{Scénář 9.3: Přepínání panelů přes File menu (Toggles)} \\
...
\textbf{Scénář 9.4: Spouštění základních Git operací přes menu} \\
...
\textbf{Scénář 9.5: Ochranné prvky u kritických Git akcí přes menu} \\
