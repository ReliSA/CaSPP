...
from utils.constants import MarkdownPreviewConstants, EditorConstants
...
class EditorManager:
    ...
    def live_preview_check_box_state_changed(self, state: int) -> None:
        ...
        logger.info("UI: live preview %s", "enabled" if is_visible else "disabled")
    ...
    def markdown_analyzer_check_box_state_changed(self, state: int) -> None:
        ...
        """
        is_visible = (state == Qt.CheckState.Checked.value)
        logger.info("UI: analyzer panel %s", "shown" if is_visible else "hidden")
        for tab in self.tab_manager.tab_states.keys():
    ...
    def _update_tab_preview(self) -> None:
