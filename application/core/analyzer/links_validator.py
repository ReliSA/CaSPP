...
class LinkValidator:
    ...
    def check_reciprocal_links(self) -> None:
        """Validates reciprocal links between patterns."""
        ...
        if len(self.warnings) == before:
            self.passed.append("All reciprocal links are present.")

    def check_link_targets(self) -> None:
        """Validates that all relative markdown links point to files that exist in the project index."""
        before = len(self.warnings)
        ...
        if len(self.warnings) == before:
            self.passed.append("All link targets exist.")
    ...
    def check_alias_consistency(self) -> None:
        ...
        if len(self.warnings) == before:
            self.passed.append("All link aliases are consistent.")


    def check_citation_keys(self) -> None:
        """Validates that citation keys used in links to References.md exist in that file."""
        ...
        if len(self.warnings) == before:
            self.passed.append("All citation keys are valid.")

    def run_all_checks(self) -> List[Dict[str, Any]]:
        """Runs all link-related validation rules.

        Returns:
            A list of warning dictionaries, each containing 'line' and 'msg' keys.
